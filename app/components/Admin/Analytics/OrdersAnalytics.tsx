...
export default function OrdersAnalytics({ isDashboard }: Props) {
  ...
  // Define analyticsData type and ensure it is properly structured
  const analyticsData: { name: string; Count: number }[] = [];

  // Use a type-safe way to populate the analyticsData array
  if (data?.orders?.last12Months) {
    data.orders.last12Months.forEach((item: OrderData) => {
