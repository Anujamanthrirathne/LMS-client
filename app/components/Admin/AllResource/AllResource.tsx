...
const AllResource = () => {
  ...
  useEffect(() => {
    console.log("API Data:", data);  // This will log the data after it's fetched
  }, [data]);
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      flex: 0.2,
      renderCell: (params) => (
        <Link href={`/admin/edit-resource/${params.row.id}`}>
  ...
  const getBase64ImageFromURL = async (imageUrl: string): Promise<string> => {
    ...
    const generatePDF = async () => {
      ...
      autoTable(doc, {
        ...
        columnStyles: {
          0: { cellWidth: 50 }, // ID
          1: { cellWidth: 50 }, // Name (Enough Space)
          2: { cellWidth: 45 }, // Email (More Space)
          3: { cellWidth: 45 }, // Role
          4: { cellWidth: 45 }, // Courses Purchased
          5: { cellWidth: 45 }, // Joined At (Fixed Space)
        },
